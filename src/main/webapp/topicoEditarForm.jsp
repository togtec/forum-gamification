...
<body>
	<div id="containerPaginaGeral">
		<fg:cabecalho />
		<h1 id="tituloPagina" class="centralizado">Editar Tópico</h1>
		...
		<div id="formVerticalContainer">
			<form action="atualizarTopico" method="post">
				<div class="campo">
					<input type="hidden" name="forumId" value="${param.forumId}" />
				</div>			
				<div class="campo">
					<input type="hidden" name="topicoId" value="${param.topicoId}" />
				</div>
				
				<div class="campo">
					<label for="forumNome">Fórum</label> 
					<input type="text" name="forumNome" value="${topico.getForumNome()}${param.forumNome}" class="readonly" readonly/>
					<p class="errorMessage" id="errorMsgForumNome">${errorMsgForumNome}</p>
				</div>												
				<div class="campo">
					<label for="topicoNome">Nome do Tópico</label> 
					<input type="text" name="topicoNome" value="${topico.getTopicoNome()}${param.topicoNome}" />
					<p class="errorMessage" id="errorMsgTopicoNome">${errorMsgTopicoNome}</p>
				</div>				
				<div class="campo">
					<label for="topicoConteudo">Conteúdo</label> 
					<textarea rows="10" name="topicoConteudo">${topico.getTopicoConteudo()}${param.topicoConteudo}</textarea>
